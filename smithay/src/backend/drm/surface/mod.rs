...
impl DrmSurface {
    ...
    /// Requests tearing (async) page flips for subsequent flips of this
    /// surface. Only honored on atomic devices supporting
    /// `DRM_CAP_ATOMIC_ASYNC_PAGE_FLIP`; refused flips fall back to
    /// synchronized presentation automatically.
    pub fn set_allow_async_page_flip(&self, allow: bool) {
        match &*self.internal {
            DrmSurfaceInternal::Atomic(surf) => surf.set_allow_async_page_flip(allow),
            DrmSurfaceInternal::Legacy(_) => {}
        }
    }
