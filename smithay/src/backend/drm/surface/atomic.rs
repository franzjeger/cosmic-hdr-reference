...
pub struct AtomicDrmSurface {
    ...
    /// Whether the device accepts `PAGE_FLIP_ASYNC` on atomic commits
    /// (`DRM_CAP_ATOMIC_ASYNC_PAGE_FLIP`), probed once at creation.
    supports_async_flip: bool,
    /// Whether the next page flips may tear. Set per frame by the compositor;
    /// only honored when the device supports async atomic flips.
    allow_async_flip: AtomicBool,
...
impl AtomicDrmSurface {
    ...
    pub fn new(
        ...
        drop(_guard);
        // DRM_CAP_ATOMIC_ASYNC_PAGE_FLIP (kernel 6.8+): whether atomic
        // commits accept PAGE_FLIP_ASYNC for tearing presentation.
        let supports_async_flip = drm_ffi::get_capability(std::os::unix::io::AsFd::as_fd(&*fd), 0x15)
            .map(|cap| cap.value != 0)
            .unwrap_or(false);
        ...
        let surface = AtomicDrmSurface {
            ...
            supports_async_flip,
            allow_async_flip: AtomicBool::new(false),
            span,
        };
    ...
    /// Requests tearing (async) page flips for subsequent [`Self::page_flip`]
    /// calls. Ignored when the device lacks `DRM_CAP_ATOMIC_ASYNC_PAGE_FLIP`;
    /// flips that the kernel refuses to perform asynchronously (e.g. ones
    /// updating more than the primary plane's framebuffer) fall back to a
    /// synchronized flip automatically.
    pub fn set_allow_async_page_flip(&self, allow: bool) {
        self.allow_async_flip
            .store(allow && self.supports_async_flip, Ordering::Relaxed);
    }
    ...
    pub fn page_flip<'a>(
        ...
        let async_flip = self.allow_async_flip.load(Ordering::Relaxed);
        ...
        if res.is_err() && async_flip {
            // The kernel rejects async flips that change anything besides the
            // primary plane's framebuffer (cursor updates, format changes);
            // present those synchronized instead of failing the frame.
            trace!("Async page flip rejected; retrying synchronized");
            res = self.fd.atomic_commit(base_flags, req.build()?);
        }
...
impl Drop for AtomicDrmSurface {
    fn drop(&mut self) {
        ...
        if let Err(err) = self.clear_state() {
            warn!(
                "Unable to clear state: {}; refreshing KMS state and retrying",
                err
            );
            if let Err(reset_err) = self.reset_state::<DrmDeviceFd>(None) {
                warn!("Unable to refresh KMS state after clear failure: {}", reset_err);
            } else if let Err(retry_err) = self.clear_state() {
                warn!("Unable to clear state after refreshing KMS state: {}", retry_err);
            }
        }
    }
}
