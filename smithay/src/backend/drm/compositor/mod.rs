...
bitflags::bitflags! {
    ...
    pub struct FrameFlags: u32 {
        ...
        /// Allow the resulting flip to tear (async page flip) when the client
        /// content permits it; flips the kernel refuses asynchronously fall
        /// back to synchronized presentation
    ...
    G: AsFd + Clone,
{
    ...
    {
        // Applies to the flip queued for this frame; the atomic surface falls
        // back to a synchronized flip when the kernel refuses to tear.
        self.surface
            .set_allow_async_page_flip(frame_flags.contains(FrameFlags::ALLOW_ASYNC_PAGE_FLIP));
